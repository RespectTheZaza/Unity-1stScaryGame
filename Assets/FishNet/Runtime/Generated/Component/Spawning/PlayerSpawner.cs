...
namespace FishNet.Component.Spawning
{
    ...
    public class PlayerSpawner : MonoBehaviour
    {
        ...
        /// <summary>
/// Sets a spawn position and rotation based on the player's ID.
/// </summary>
private void SetSpawn(Transform prefab, out Vector3 pos, out Quaternion rot)
{
    int playerID = _networkManager.ServerManager.Clients.Count - 1; // Get the current player count

    // Ensure there are enough spawn points
    if (Spawns.Length < 4)
    {
        Debug.LogError("Not enough spawn points assigned! Ensure there are at least 4.");
        SetSpawnUsingPrefab(prefab, out pos, out rot);
        return;
    }

    // Assign first two players to Team A, last two to Team B
    Transform spawnPoint = (playerID % 2 == 0) ? Spawns[playerID / 2] : Spawns[2 + (playerID / 2)];
